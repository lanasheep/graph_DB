...
def build_automata(prods, automata_intersec=None):
    ...
    sz = 0
    for prod in prods:
        if automata_intersec is not None:
            automata = automata_intersec
        else:
...
def tensor_alg(prods, graph, automata_intersec=None):
    matrix_automata, start_set, final_set, edges, eps_nonterms, nonterms, m = build_automata(prods, automata_intersec)
